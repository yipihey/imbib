...
public actor GroupFeedRefreshService {
    ...
    private func processResultsForInboxByID(_ results: [SearchResult], smartSearchID: UUID) async -> Int {
        ...
        // Add existing publications to the result collection (so they show in the feed view)
        if !existingPubs.isEmpty, let collection = resultCollection {
        ...
        // Return total papers in the feed (existing + new)
        return existingPubs.count + newPaperIDs.count
    }
    ...
    private func processResultsForInbox(_ results: [SearchResult], smartSearch: CDSmartSearch) async -> Int {
        ...
        if !newPapers.isEmpty {
            ...
            // Immediate ADS enrichment to resolve bibcodes for Similar/Co-read features
            // This uses a single batch API call for efficiency
            let enrichedCount = await EnrichmentCoordinator.shared.enrichBatchImmediately(newPapers)
            Logger.inbox.infoCapture(
        ...
        // Return total papers in the feed (existing + new)
        return existingPubs.count + newPapers.count
    }
