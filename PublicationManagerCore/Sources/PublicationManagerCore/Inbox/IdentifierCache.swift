...
///     cache.add(pub)  // Keep cache current
/// }
/// ```
public actor IdentifierCache {
    ...
    /// Add a newly created publication's identifiers to the cache.
    ///
    /// Call this after creating a new publication to keep the cache current
    /// within the same batch operation.
    public func add(_ publication: CDPublication) {
        if let doi = publication.doi?.lowercased() {
            dois.insert(doi)
        }
        if let arxivID = publication.arxivIDNormalized {
            arxivIDs.insert(arxivID)
        }
        if let bibcode = publication.bibcodeNormalized {
            bibcodes.insert(bibcode)
        }
        if let ssID = publication.semanticScholarID {
            semanticScholarIDs.insert(ssID)
        }
        if let oaID = publication.openAlexID {
