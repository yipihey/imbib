...
public struct ExpandableAuthorList: View {
    ...
    private var collapsedView: some View {
        ...
        HStack(spacing: 0) {
            ...
            .buttonStyle(.plain)
            #if os(macOS)
            .help("Show all \(authors.count) authors (\(hiddenCount) hidden)")
            #endif

            // Last author (if shown)
