...
extension ADSSource: EnrichmentPlugin {
    ...
    private func fetchBasicInfo(bibcodeQuery: String) async throws -> (BasicInfo, String) {
        ...
        let arxivID = identifiers?.first { $0.hasPrefix("arXiv:") }?.replacingOccurrences(of: "arXiv:", with: "")
    ...
    /// - Single API call for up to 50 papers (vs 50 calls)
    ...
    public func enrichBatch(
        ...
        Logger.sources.info("ADS: batch enriching \(requests.count) papers")
        ...
        for request in requests {
            if let bibcodeQuery = try? resolveBibcode(from: request.identifiers) {
                ...
                queryTerms.append(bibcodeQuery)
            }
        }
        ...
        // Build OR query: bibcode:(term1 OR term2 OR term3)
        // For DOI/arXiv, we use identifier: syntax which is more flexible
        let query = "identifier:(" + queryTerms.joined(separator: " OR ") + ")"

        do {
            let batchResults = try await fetchBasicInfoBatch(query: query, maxRows: requests.count)
            ...
            for (bibcode, info) in batchResults {
                ...
                // Check both direct bibcode match and query term match
            ...
            for request in requests {
                if results[request.publicationID] == nil {
                    results[request.publicationID] = .failure(EnrichmentError.notFound)
                }
            }

            Logger.sources.info("ADS: batch enrichment complete - \(batchResults.count)/\(requests.count) found")
            return results

        } catch {
            Logger.sources.error("ADS: batch enrichment failed: \(error.localizedDescription)")
            // Return same error for all requests
            return Dictionary(uniqueKeysWithValues: requests.map {
                ($0.publicationID, .failure(error))
            })
        }
    }
