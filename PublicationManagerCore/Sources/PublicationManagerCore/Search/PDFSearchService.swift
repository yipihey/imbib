...
public actor PDFSearchService {
    ...
    public func search(
        ...
        // Build list of PDFs to search
        var pdfURLs: [(publicationID: UUID, url: URL)] = []
        ...
        for publication in publications {
            ...
            // Resolve the PDF URL
            if let url = resolvePDFURL(linkedFile: pdfFile, library: library) {
                pdfURLs.append((publication.id, url))
            }
        }
    ...
    public func contains(query: String, in publication: CDPublication, library: CDLibrary?) async -> Bool {
        guard !query.isEmpty,
              ...
              let url = resolvePDFURL(linkedFile: pdfFile, library: library) else {
            return false
        }

        return await provider.contains(query: query, in: url)
    }
